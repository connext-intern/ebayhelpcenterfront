        ...
        run: function () {
            ...
            this.vue = new Vue({
                ...
                methods: {
                    ...
                    loadList: function () {
                        var that = this;
                        Api.set({ key: 'getAllMessage', type: 'GET', data: { pageNo: this.pagerObj.current, pageSize: this.pagerObj.pagesize } }, {
                            success: function (data, params) {
                                if (data.code == Api.getData.getCode().success) {
                                    that.list = data.result;
                                    if (that.list && that.list.length) that.pager(data.result.totalPages, data.result.totalCounts);
                                };
                            }
                        });
                    }
